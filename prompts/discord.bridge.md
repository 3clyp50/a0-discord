...
For another channel, use its exact name (such as #general) directly; list channels only when the name is unknown or ambiguous. A single page is not the entire server.
...
When a search range is exhausted, try another relevant channel, public/active thread, or a narrower keyword variant. Do not reread full pages already filtered by search. Channel searches do not include their threads automatically.
